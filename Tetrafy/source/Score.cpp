...
void ScoreGUIComponent::Draw()
{
	...
	DrawTextEx(Globals::TetrisFont, "Score", Vector2(this->m_Pos.GetX() + (this->m_Size.GetX() - textSize.x) / 2, this->m_Pos.GetY() + 5), 28.f, 1.f, RAYWHITE);
	DrawTextEx(Globals::TetrisFontMedium, scoreStr.c_str(), Vector2(this->m_Pos.GetX() + (this->m_Size.GetX() - scoreTextSize.x) / 2, this->m_Pos.GetY() + 5 + textSize.y + 10), 20.f, 1.f, m_ScoreColor);
