...
{
	Vector2 textSize = MeasureTextEx(Globals::TetrisFont, "Score", 28.f * Globals::Options.GUIScale, 1.f);
	std::string scoreStr = std::to_string(m_Score);
	Vector2 scoreTextSize = MeasureTextEx(Globals::TetrisFontMedium, scoreStr.c_str(), 20.f * Globals::Options.GUIScale, 1.f);

	DrawRectangleLines(m_Pos.GetX() - 5, m_Pos.GetY() - 5, m_Size.GetX() + 5, m_Size.GetY() + 5, RAYWHITE);
	DrawTextEx(Globals::TetrisFont, "Score", Vector2((float)m_Pos.GetX() + ((float)m_Size.GetX() - textSize.x) / 2, (float)m_Pos.GetY() + 5), 28.f * Globals::Options.GUIScale, 1.f, RAYWHITE);
	DrawTextEx(Globals::TetrisFontMedium, scoreStr.c_str(), Vector2((float)m_Pos.GetX() + ((float)m_Size.GetX() - scoreTextSize.x) / 2, (float)m_Pos.GetY() + 5 + textSize.y + 10), 20.f * Globals::Options.GUIScale, 1.f, m_ScoreColor);
