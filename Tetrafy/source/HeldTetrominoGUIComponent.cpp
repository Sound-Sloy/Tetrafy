...
{
	Vector2 textSize = MeasureTextEx(Globals::TetrisFont, "Hold", 28.f * Globals::Options.GUIScale, 1.f);

	DrawRectangleLines(this->m_Pos.GetX() - 5, this->m_Pos.GetY() - 5, this->m_Size.GetX() + 5, this->m_Size.GetY() + 5, RAYWHITE);
	DrawTextEx(Globals::TetrisFont, "Hold", Vector2((float)this->m_Pos.GetX() + (this->m_Size.GetX() - textSize.x) / 2, (float)this->m_Pos.GetY() + 5), 28.f * Globals::Options.GUIScale, 1.f, RAYWHITE);
